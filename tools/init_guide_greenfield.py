    ...
    *,
    ...
    for i in range(1, 4):
        if i == 1:
            ...
            row["faq_1_answer"] = (
                "greenfield 執筆プログラムの優先順（exam-format-overview → goukaku-kijun → study-plan-working 等）に沿って順次公開します。"
                "試験形式は全44問·400点満点·各出題範囲40％足切りを正本とし、"
                f"{norm(facts.get('organizer'))}の公式要項で都度確認します。"
            )
        else:
...
def run_init(
    ...
    *,
    ...
    if do_retire and retire_map:
        ...
        print(f"retired: removed={rstats['removed']} related_patched={rstats['related_patched']}")
